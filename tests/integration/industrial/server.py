...
def service_types(name: str | None = None):
...
def physical_types(name: str | None = None):
...
def get_indicators(scenario_id: int, indicator_ids: list[int] | None = None):
...
def zone_sources(scenario_id: int):
...
def zones(scenario_id: int, source: str, year: int):
...
def territory_normatives(territory_id: int):
