...
class UrbanDataQaRequestDTO(SimpleRequestDTO):
    """
    DTO for the urban-data QA endpoint.

    The agent calls tools from the external, grouped Urban MCP server to answer the
    natural-language question and, when the tools return spatial data, surfaces it as
    layers; scenario_id / chat_id are optional and used for chat-history persistence.
    Attributes:
