...
class PzzAgentExecutor:
    """Executor for A2A PZZ compliance tasks."""
    ...
    async def execute(
        ...
        async for _ in self._run_pipeline(execution, pzz_mcp_client, token):
            pass
    ...
    async def stream(
        ...
        yield task
        async for event in self._run_pipeline(execution, pzz_mcp_client, token):
            yield event
    ...
    async def _run_pipeline(
        ...
        status = self.task_store.set_status(
            task_id,
            TaskState.WAITING,
            self._agent_message(context_id, task_id, "Запуск агента по ПЗЗ."),
        )
        yield self._status_update(task_id, context_id, status, final=False)

        try:
            async for item in self.pzz_service.run_pzz_pipeline(
                ...
                request_id=execution["task_id"],
                inputs=PzzInputs.model_validate(
                    execution["metadata"].get("inputs")
                    or {
                        key: value
                        for key, value in execution["metadata"].items()
                        if key in PzzInputs.model_fields
                    }
                ),
                ...
                if event is None:
                    continue
                yield event
                if item.get("type") in {"error", "clarification"}:
                    return
    ...
    def _pipeline_item_to_event(
            ...
            status = self.task_store.set_status(
                ...
                TaskState.INPUT_REQUIRED,
                {
                    **self._agent_message(
                        context_id,
                        task_id,
                        content.get("question", "Требуется уточнение"),
                    ),
                    "parts": [
                        {
                            "type": "text",
                            "text": content.get("question") or "Требуется уточнение",
                        },
        ...
        if item_type in {"object_zone_fit", "classify_summary", "feature_collection"}:
            artifact = {
                "artifactId": f"pzz-{item_type}",
                "name": item_type,
    ...
    def _text_artifact(text: str, iteration: int) -> A2AData:
        ...
        return {
            "artifactId": f"pzz-answer-{iteration}",
            "name": "pzz-response",
            "description": "Ответ агента по ПЗЗ",
            "parts": [{"type": "text", "text": text}],
            "metadata": {
