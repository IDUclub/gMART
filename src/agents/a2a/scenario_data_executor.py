...
class ScenarioDataAgentExecutor:
    """Execute scenario-data pipelines and map their SSE items to A2A events."""
    ...
    async def execute(
        ...
        async for _ in self._run_pipeline(execution, urban_mcp_client, token):
            pass
    ...
    async def stream(
        ...
        yield task
        async for event in self._run_pipeline(execution, urban_mcp_client, token):
            yield event
    ...
    async def _run_pipeline(
        ...
        status = self.task_store.set_status(
            ...
            TaskState.WAITING,
            self._agent_message(
                context_id, task_id, "Starting the scenario-data pipeline."
            ),
        )
        yield self._status_update(task_id, context_id, status, final=False)

        try:
            async for item in self.scenario_data_service.run_scenario_data_pipeline(
            ...
            status = self.task_store.set_status(
                ...
                TaskState.COMPLETED,
                self._agent_message(
                    context_id, task_id, "Scenario-data pipeline completed."
                ),
            )
            ...
            status = self.task_store.set_status(
                ...
                TaskState.FAILED,
                self._agent_message(
                    context_id, task_id, f"Scenario-data pipeline failed: {exc}"
                ),
            )
            yield self._status_update(task_id, context_id, status, final=True)
            if not emitted_text:
                artifact = self._text_artifact(
                    "Scenario-data pipeline failed.", append=False
                )
        ...
        if item_type == "error":
            status = self.task_store.set_status(
                ...
                self._agent_message(
                    context_id,
                    task_id,
                    content.get("message", "Scenario-data pipeline error"),
                ),
            )
    ...
    def _prepare_execution(self, params: A2AData) -> A2AData:
        ...
        scenario_id = self._resolve_optional_scenario_id(request_data, raw_text)
    ...
    @classmethod
    def _resolve_optional_scenario_id(
        cls, request_data: A2AData, raw_text: Any
    ) -> int | None:
    ...
    def _extract_scenario_id_from_text(text: str) -> int | None:
        match = re.search(
            r"\b(?:scenario_id|scenarioId)\b\s*[:=]\s*(\d+)",
            text,
            re.IGNORECASE,
        )
    ...
    def _extract_request_data(params: A2AData, message: A2AData) -> A2AData:
        ...
        for key in ("model", "temperature", "request", "userQuery", "chat_id"):
            if key in params:
                data[key] = params[key]
    ...
    @classmethod
    def _sanitize_user_message(cls, message: A2AData) -> A2AData:
        parts = [
            {"type": "text", "text": cls._hide_inline_ids(str(part["text"]))}
            for part in message.get("parts", [])
            if isinstance(part, dict) and part.get("text")
        ]
    ...
    def _hide_inline_ids(text: str) -> str:
        text = re.sub(r"\bscenario_?id\b\s*[:=]\s*\d+", "", text, flags=re.I)
    ...
    def _text_artifact(text: str, append: bool) -> A2AData:
        return {
            "artifactId": "scenario-data-agent-text",
            "name": "scenario-data-agent-response",
            "description": "Text response from the scenario-data agent",
            "parts": [{"type": "text", "text": text}],
    ...
    def _table_artifact(table_content: A2AData) -> A2AData:
        ...
        return {
            ...
            "description": "Strict table produced by the scenario-data pipeline",
            "parts": [
                {
                    "type": "data",
                    "data": table_content,
    ...
    def _geojson_artifact(layer_name: str, feature_collection: A2AData) -> A2AData:
        ...
        return {
            ...
            "name": layer_name,
            "description": "GeoJSON layer produced by the scenario-data pipeline",
            "parts": [
                {
                    "type": "data",
