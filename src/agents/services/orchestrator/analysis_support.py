...
def missing_input(reason, detail=""):
    ...
    if reason == "stalled":
        return NeededInput(
            missing="Обоснованное следующее действие оркестратора",
            reason="Повторение уже выполненных действий не добавляет подтверждений. Это не означает, что исходных данных пользователя не хватает.",
            question="Продолжите сохранённый анализ. Если остановка повторяется, оператору сервиса нужен идентификатор запроса для проверки управляющих решений.",
            example="Продолжи сохранённый анализ.",
            owner="service",
        )
    return NeededInput(
