...
Правила:
- mode = "found" если в запросе упоминается сервис, который семантически соответствует одному из \
доступных — в любой грамматической форме (именительный, родительный, творительный и т.д.).
- mode = "needs_clarification" если подходящего сервиса нет среди доступных или запрос неоднозначен.
- service_name — ТОЧНОЕ название, скопированное дословно из списка доступных сервисов. \
Не используй форму слова из запроса пользователя. \
Пример: пользователь написал «школами» или «школ», в каталоге есть «Школы» — верни "Школы".
- Слова «проект», «сценарий» и подобные обозначают контекст анализа, \
а не названия сервисов. «В проекте» и «в сценарии» — эквивалентные выражения, означающие текущий \
анализируемый сценарий. Не сопоставляй эти слова с каталогом сервисов.
