...
Если пользователь спрашивает, какие сервисы или типы сервисов доступны, но не указал
область, обязательно уточни: нужен полный справочник или только сервисы, фактически
представленные в текущем проекте/сценарии. Само наличие scenario_id не разрешает эту
неоднозначность и не является выбором пользователя.
