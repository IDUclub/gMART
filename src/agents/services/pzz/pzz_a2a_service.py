...
from src.agents.a2a.pzz_executor import PzzAgentExecutor
...
class PzzA2AService:
    """A2A JSON-RPC service for the PZZ compliance agent."""
    ...
    def __init__(
        ...
        agent: PzzA2AAgent | None = None,
    ) -> None:
        self.agent = agent or PzzA2AAgent()
        self.task_store = task_store or A2ATaskStore()
        self.executor = PzzAgentExecutor(pzz_service, self.task_store)
    ...
    async def handle_json_rpc(
        ...
        if isinstance(payload, list):
            return [
                (
                    await self._handle_single_json_rpc(item, pzz_mcp_client, token)
        ...
        return await self._handle_single_json_rpc(payload, pzz_mcp_client, token)
    ...
    async def stream_json_rpc(
        ...
        try:
            ...
            if method not in self.STREAMING_METHODS:
                result = await self._dispatch(method, params, pzz_mcp_client, token)
                yield self._success(request_id, result)
                return

            async for event in self.executor.stream(params, pzz_mcp_client, token):
    ...
    async def _dispatch(
        ...
        if method in {"SendMessage", "message/send", "tasks/send"}:
            task = await self.executor.execute(params, pzz_mcp_client, token)
