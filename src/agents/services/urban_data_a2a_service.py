...
class UrbanDataA2AService:
    """A2A JSON-RPC service for the urban-data QA (external, grouped Urban MCP) agent."""
    ...
    def __init__(
        ...
        agent: UrbanDataA2AAgent | None = None,
    ) -> None:
        self.agent = agent or UrbanDataA2AAgent()
        self.task_store = task_store or A2ATaskStore()
        self.executor = UrbanDataAgentExecutor(urban_data_qa_service, self.task_store)
    ...
    async def handle_json_rpc(
            ...
            return [
                (
                    await self._handle_single_json_rpc(
                        item, urban_data_mcp_client, token
                    )
        ...
        return await self._handle_single_json_rpc(payload, urban_data_mcp_client, token)
    ...
    async def _handle_single_json_rpc(
        ...
        try:
            ...
            method = payload.get("method")
            params = self._extract_params(payload)
            result = await self._dispatch(method, params, urban_data_mcp_client, token)
    ...
    async def _dispatch(
        ...
        if method in {"SendMessage", "message/send", "tasks/send"}:
            task = await self.executor.execute(params, urban_data_mcp_client, token)
