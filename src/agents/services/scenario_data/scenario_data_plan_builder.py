...
Workspace-каталог: {json.dumps(WORKSPACE_TOOL_CATALOG, ensure_ascii=False)}"""
    ...
    def _shortlist(
        ...
        *,
        ...
        # Keep both ontologies and their scenario dictionaries reachable even when
        # the user's inflected noun does not occur in tool descriptions.
        core_names = {
            "GetScenarioServices",
        ...
        if any(marker in user_query.lower() for marker in ("карт", "геометр", "сло")):
            core_names.update(
                {
                    "GetScenarioServicesWithGeometry",
                    "GetScenarioPhysicalObjectsWithGeometry",
        ...
        for tool in tools:
            if tool.name in core_names and tool.group == "projects":
                chosen[(tool.group, tool.name)] = tool
