...
def _classify_text(
    ...
    # A named type is a filter, not a request for the whole type distribution.
    # Likewise, additional predicates must be handled by the general planner.
    if re.search(r"\bтипа\s+(?!есть\b|в\b|по\b)\S+", lowered) or re.search(
        r"[«\"]|этаж|радиус|вместим|по адресу|больше|меньше|выше|ниже|контекст|окружен",
        lowered,
    ):
        return None
    has_entity = any(
