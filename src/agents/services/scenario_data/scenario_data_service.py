...
class ScenarioDataService(BaseLlmService):
    ...
    async def _run_scenario_data_pipeline(
        ...
        if broad_requested:
            ...
            return
        if analytics_requested:
            async for event in ScenarioAnalytics(self).run(
                request_id=request_id,
                client=urban_mcp_client,
                token_ref=token_ref,
                model=model,
        ...
        for attempt in range(MAX_ANSWER_ATTEMPTS):
            ...
            for _ in range(MAX_SCENARIO_TOOL_CALLS + 3 if tools else 0):
                ...
                if action.action == ScenarioDataActionKind.FINAL_ANSWER:
                    if (
                        successful_calls == 0
                        and not asked_to_fetch
                        and (
                            attempt > 0
                            or not any(item.get("retrieved") for item in observations)
                        )
                    ):
                        asked_to_fetch = True
                        ...
                        observations.append(
                            {
                                ...
                                "summary": (
                                    "Нельзя делать выводы об объектах без успешной выборки. "
                                    "Выбери источник, соответствующий типу и области запроса. "
                                    "Отсутствие наблюдений не означает ноль объектов."
                                ),
                            }
                        )
                        continue
                ...
                for path, feature_collection in self._feature_collections(result):
                    layer_count += 1
                    # A planner's proposed label is not evidence of a type filter.
                    name = tool.title
                    if path:
                        name = f"{name} · {path}"
                ...
                observation: dict[str, Any] = {
                    ...
                    "retrieved": True,
                    "source_role": (
                        "справочник типов, количество записей не равно количеству объектов"
                        if "Types" in action.tool_name or action.group == "dictionaries"
                        else "данные сущностей"
                    ),
                    "source_scope": (
                        "окружение сценария"
                        if "Context" in action.tool_name
                        else (
                            "выбранный сценарий"
                            if "Scenario" in action.tool_name
                            else "общие данные"
                        )
                    ),
                    "arguments": arguments,
            ...
            if attempt == MAX_ANSWER_ATTEMPTS - 1:
                answer = (
                    "Не удалось подтвердить ответ на запрос по полученным данным. "
                    "Проверка выявила недостаточность или несогласованность результата. "
                    "Уточните тип данных и условия выборки."
                )
    ...
    async def _run_entity_query_pipeline(
        self,
        *,
        request_id,
        urban_mcp_client,
        ...
        if any(
            ...
            return
        # Older MCP catalogues without explicit type filters use the general planner.
        if any(
            f"{domain}_id"
            not in named[("projects", names[1])].input_schema.get("properties", {})
        ...
        async def execute(tool, arguments, box):
            ...
            async for event in self._retryable_operation(
                ...
                token_ref,
                lambda: urban_mcp_client.execute_tool(
                    tool.group, tool.name, prepared, meta={"scenario_id": scenario_id}
                ),
                box,
                retry_transient=True,
            ):
        ...
        try:
            ...
            for domain, (catalog_name, _) in domains.items():
                ...
                async for event in execute(
                    named[("projects", catalog_name)], {"for_context": False}, box
                ):
                    yield event
            ...
            if selection.candidate is None:
                ...
                name = candidate["name"]
                entity_name = domains[domain][1]
                entity_tool = named[("projects", entity_name)]
                arguments = {f"{domain}_id": candidate["type_id"]}
                box = []
                async for event in execute(entity_tool, arguments, box):
                    yield event
                records = verified_entity_records(
                    self._unwrap_result(box[0]), candidate
                )
                count = len(records)
                noun = "сервисов" if domain == "service_type" else "физических объектов"
                answer = f"В выбранном сценарии {noun} типа «{name}»: {count}."
                if request.operation == "count":
                    table = {
                        "name": "scenario_entity_count",
                        "title": f"Количество: {name}",
                        "columns": [
                            {"key": "type_name", "label": "Тип"},
                            {"key": "count", "label": "Количество"},
                        ],
                        "rows": [{"type_name": name, "count": count}],
                        "total_rows": 1,
                        "complete": True,
                    }
                else:
                ...
                if table is not None:
                    ...
                    if not table.get("complete", True):
                        answer += f" В таблице показаны первые {len(table['rows'])} из {count} записей."
                if request.operation == "map":
                    geometry_tool = named.get(
                        ("projects", entity_name + "WithGeometry")
                    )
                    if geometry_tool is None:
                        answer += (
                            " Инструмент геометрии недоступен; карта не сформирована."
                        )
                    else:
                        geo_box = []
                        async for event in execute(geometry_tool, arguments, geo_box):
                            yield event
                        geo = self._unwrap_result(geo_box[0])
                        geo_records = verified_entity_records(geo, candidate)
                        id_field = domain.removesuffix("_type") + "_id"
                        if {r[id_field] for r in records} != {
                            r[id_field] for r in geo_records
                        }:
                            raise ValueError(
                                "Map and entity query have different entity IDs"
                            )
                        if (
                            not isinstance(geo, dict)
                            or geo.get("type") != "FeatureCollection"
                        ):
                            raise ValueError(
                        ...
                        artifacts.append(
                            {
                                ...
                                "content": {
                                    "name": name,
                                    "feature_collection": geo,
                                },
                            }
                        )
                        answer += f" На карте показаны объекты этого типа ({count}); список приведён в таблице."
        except ValueError as exc:
            logger.warning(f"Scenario entity query could not be verified: {exc}")
            artifacts.clear()
            answer = (
                "Не удалось подтвердить выборку по указанному типу: результат не прошёл проверку. "
                "Уточните название типа и условия запроса."
            )
        handled.append(True)
...
{context}""",
    ...
    def _result_summary(cls, value: Any) -> str:
        rows = cls._unwrap_result(value)
        if (
            isinstance(rows, list)
            and rows
            and isinstance(rows[0], dict)
            and isinstance(rows[0].get("indicator"), dict)
        ):
            sid = (rows[0].get("scenario") or {}).get("id")
            if sid is not None:
                facts = normalize_indicators(rows, sid)
                return json.dumps(
                    {
                        "total_records": len(facts),
                        "facts": facts[:8],
                        "complete": len(facts) <= 8,
                    },
                    ensure_ascii=False,
                    default=str,
                )
