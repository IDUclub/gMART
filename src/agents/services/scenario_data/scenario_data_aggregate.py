...
def aggregate_records(records: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    for path, counter in counters.items():
        ...
        if not _is_category(path) and distinct > max(
            ...
            continue
        # Entity type identity must survive verbose address/building metadata.
        rank = (
            0
            if path in {"physical_object_type.name", "service_type.name"}
            else 1 if _is_category(path) else 2
        )
        scored.append((rank, distinct, path, counter))
