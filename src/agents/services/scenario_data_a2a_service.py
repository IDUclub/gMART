...
class ScenarioDataA2AService:
    """A2A JSON-RPC service for read-only Urban API data questions."""
    ...
    def __init__(
        ...
        agent: ScenarioDataA2AAgent | None = None,
    ) -> None:
        self.agent = agent or ScenarioDataA2AAgent()
        self.task_store = task_store or A2ATaskStore()
        self.executor = ScenarioDataAgentExecutor(
            scenario_data_service, self.task_store
        )
    ...
    async def handle_json_rpc(
        ...
        if isinstance(payload, list):
            return [
                (
                    await self._handle_single_json_rpc(item, urban_mcp_client, token)
        ...
        return await self._handle_single_json_rpc(payload, urban_mcp_client, token)
    ...
    async def stream_json_rpc(
        ...
        try:
            ...
            if method not in self.STREAMING_METHODS:
                result = await self._dispatch(method, params, urban_mcp_client, token)
                yield self._success(request_id, result)
                return
            async for event in self.executor.stream(params, urban_mcp_client, token):
    ...
    async def _dispatch(
        ...
        if method in {"SendMessage", "message/send", "tasks/send"}:
            task = await self.executor.execute(params, urban_mcp_client, token)
