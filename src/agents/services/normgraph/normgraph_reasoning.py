...
Числовая метка [1] является ссылкой на заголовок [1] со всеми его реквизитами;
не требуй повторять их только ради оформления. Если запрошена сама запись ограничения,
проверь её restriction_id по id из контекста. Не требуй ID объектов, которых нет в источнике.
