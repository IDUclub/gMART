...
class ComplianceA2AService:
    """A2A JSON-RPC service for the normative compliance-check agent."""
    ...
    def __init__(
        ...
        agent: ComplianceA2AAgent | None = None,
    ) -> None:
        self.agent = agent or ComplianceA2AAgent()
        self.task_store = task_store or A2ATaskStore()
        self.executor = ComplianceAgentExecutor(restriction_service, self.task_store)
    ...
    async def handle_json_rpc(
        ...
        if isinstance(payload, list):
            return [
                (
                    await self._handle_single_json_rpc(item, clients, token)
        ...
        return await self._handle_single_json_rpc(payload, clients, token)
    ...
    async def stream_json_rpc(
        ...
        try:
            ...
            if method not in self.STREAMING_METHODS:
                result = await self._dispatch(method, params, clients, token)
                yield self._success(request_id, result)
                return

            async for event in self.executor.stream(params, clients, token):
    ...
    async def _dispatch(
        ...
        if method in {"SendMessage", "message/send", "tasks/send"}:
            task = await self.executor.execute(params, clients, token)
