...
class NormGraphA2AService:
    """A2A JSON-RPC service for the normative-restrictions QA (NormGraph graph-RAG) agent."""
    ...
    def __init__(
        ...
        agent: NormGraphA2AAgent | None = None,
    ) -> None:
        self.agent = agent or NormGraphA2AAgent()
        self.task_store = task_store or A2ATaskStore()
        self.executor = NormGraphAgentExecutor(normgraph_rag_service, self.task_store)
    ...
    async def handle_json_rpc(
            ...
            return [
                (
                    await self._handle_single_json_rpc(
                        item, normgraph_mcp_client, token
                    )
        ...
        return await self._handle_single_json_rpc(payload, normgraph_mcp_client, token)
    ...
    async def _handle_single_json_rpc(
        ...
        try:
            ...
            method = payload.get("method")
            params = self._extract_params(payload)
            result = await self._dispatch(method, params, normgraph_mcp_client, token)
    ...
    async def _dispatch(
        ...
        if method in {"SendMessage", "message/send", "tasks/send"}:
            task = await self.executor.execute(params, normgraph_mcp_client, token)
