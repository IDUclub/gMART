...
class DvdMcpClient(BaseMcpClient):
    ...
    Exposed tools: ``search_texts``, ``search_tables``, ``search_all`` (vector search),
    ``job_status`` and ``document_versions``. Only the search tools are used for RAG.
    """
    ...
    async def search(
        ...
        Args:
            ...
            name (str | None): Optional document name filter.
