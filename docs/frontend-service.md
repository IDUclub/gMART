...
Встроенный интерфейс показывает каждый SSE-статус карточкой непосредственно в ленте
диалога и сохраняет раскрываемую временную шкалу текущего запуска. История загружается
из ChatStorage глобально и фильтруется на клиенте по тексту, сценарию, проекту и агенту.
Сохранённые `tool_call` не выполняются автоматически: пользователь явно нажимает
«Восстановить слои». Слои можно удалить по одному или очистить все с отменой; таблицы
при очистке карты сохраняются.
...
Консоль работает только с фиксированными источниками `idu`, `urban`, `effects`, `dvd`
и `normgraph`:

- `GET /mcp-diagnostics/sources` — доступность источников;
- `GET /mcp-diagnostics/tools?source=urban` — безопасный каталог инструментов;
- `GET /mcp-diagnostics/prompts?source=dvd` — промпты выбранного сервера;
- `POST /mcp-diagnostics/tools/call` — прямой запуск инструмента с полями `source`,
  `group`, `name`, `arguments` и `meta`.

Произвольные MCP URL не принимаются. Каталог и запуск ограничены инструментами с
`readOnlyHint=true`; для старых серверов без аннотаций применяется консервативный
список префиксов чтения и расчёта. Перед вызовом разрешение проверяется повторно.
...
Скачивает лог-файл текущего запуска приложения без авторизации. Используется для диагностики.

### `POST /system/config`

Возвращает текущую конфигурацию сервиса без авторизации и системного пароля. Тело запроса
необязательно: старый формат `{"password": "..."}` принимается, но пароль игнорируется.
...
Запрос может сузить проверку темой («по школам») и документом («из СП 42.13330»).
Если документ не определён однозначно, поток завершается событием `clarification`:
`content.question` — готовый текст с нумерованным списком, `content.options` —
`[{number, label, value}]` для кнопок выбора (необязательно). Ответ пользователя
(«2», «1, 3», «все», название документа) отправляется **следующим запросом в тот же
`chat_id`** — сервер сам продолжит исходную проверку с выбранными документами.
Пока выбор ожидается, UI ничего дополнительно хранить не нужно. Подробнее —
[`compliance-agent.md`](compliance-agent.md#область-проверки-темы-и-документы).
...
Новые сообщения compliance в ChatStorage содержат только `tool_call` и `text`.
Статусы и структурированные результаты проверки доступны в SSE-потоке и
Redis-журнале запроса, но не сохраняются в истории чата. Старые сообщения могут
содержать `compliance_result`, который UI отображает как структурированную
...
| --- | --- | --- |
| `provision_summary` | Сводка по сервисам | `service`, `capacity`, `demand`, `deficit`, `surplus`, `balance`; при неполных результатах также `status` («Рассчитано» / «Нет данных»). Рассчитанные строки отсортированы по убыванию дефицита, услуги без данных идут в конце с числовыми значениями `null`, не `0`. |
...
Если модель вернула неполный или не соответствующий формату ответ, `message` равен
`"Model response error"`, а предшествующий `chunk` предлагает повторить или переформулировать
запрос: это не сбой сервера, повтор обычно проходит.

Если истёк срок выполнения пайплайна, `message` равен `"Pipeline deadline exceeded"`, а
предшествующий `chunk` сообщает, сколько длилась работа. Такой запрос автоматически не
перезапускается: повтор упёрся бы в ту же перегрузку модели.

#### Сроки выполнения и загрузка LLM

Сроки агентов scenario-data (5 минут активной работы, 15 минут всего) и document-QA
(около 14,5 минуты) заданы для обычной скорости LLM. Все агенты делят один сервер модели,
поэтому сервис по каждому вызову LLM оценивает, во сколько раз сервер сейчас медленнее
обычного: сравнивает фактическое время ответа с ожидаемым по числу токенов при
`LLM_NOMINAL_TOKENS_PER_SECOND` (по умолчанию 100 ток/с). Учитываются вызовы за последние
10 минут и ещё не завершённые вызовы, которые идут дольше, чем занял бы весь их лимит
токенов. Каждая секунда работы засчитывается в срок с делением на это замедление, так что
срок растягивается только на время перегрузки. Растяжение ограничено
`LLM_DEADLINE_MAX_FACTOR` (по умолчанию 4): при любой загрузке 5-минутный срок длится не
больше 20 минут. Пока запрос работает, его состояние, журнал событий и блокировка чата в
Redis продлеваются, поэтому переподключение по `request_id` работает и после 15 минут.
