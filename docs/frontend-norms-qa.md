...
1. **retrieval_planning** — LLM выбирает инструмент NormGraph: `search_restrictions` для открытых
   ...
   Для вопросов о размещении объекта («расстояние», «вокруг», «рядом», «доступность», СЗЗ…)
   запрос формулируется лексикой размещения, а поиск ограничивается видами
   `минимальное_расстояние`, `требование_размещения`, `запрет_размещения`. Иначе на вопрос
   «где можно размещать детский сад» находились требования к его помещениям. Если текстовый
   поиск с этим фильтром ничего не нашёл, следующая итерация ищет без него.
...
4. **answer_drafting** — ответ стримится чанками; каждый чанк помечен номером черновика
   `iteration`. Ответ обязан ссылаться на источники по каждому приведённому ограничению и
   включает только нормы по предмету вопроса. Контекст модели разделяет строку «Структура»
   (извлечённая тройка, служебная) и «Текст пункта» (единственный текст, который можно
   цитировать в «»). Ответ оформляется списком: если модель всё же пишет markdown-таблицу, её
   строки придерживаются до конца таблицы и приходят клиенту уже пунктами списка.
5. **self_review** — сначала программа сверяет числа с единицами (м, км, %, га…) из черновика с
   контекстом: значение, которого там нет, отклоняет черновик без вызова LLM. Затем критик-LLM
   проверяет обоснованность, ссылки на источники, отсутствие толкований и расшифровок не из
   контекста, соответствие предмету вопроса, дословность цитат и упоминание найденных
   противоречий. Если ответ не устроил — формируется уточнённый запрос,
