...
Если `restrictions_applicable` ничего не нашёл, следующая итерация — `search_restrictions`
по тексту запроса без фильтров по объекту и субъекту: такого объекта нет в графе, и повтор
того же запроса ничего не даст. Если LLM не смогла построить план (например, исчерпала
бюджет вывода на рассуждение), итерация выполняет тот же текстовый поиск, а не завершает
пайплайн ошибкой.
