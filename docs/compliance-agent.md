...
```text
запрос → область проверки (темы → сущности NormGraph, документы)
...
1. LLM только разбирает запрос: `topics` (виды объектов в именительном падеже) и
   `documents` (как их назвал пользователь). Без тем и документов остаётся полная
   проверка.
2. Темы → сущности. NormGraph `resolve_entities` возвращает кандидатов (точное имя,
   алиас, основа слова, ближайшие по эмбеддингу) с числом норм и исполнимых норм.
   ...
   совпадения по имени или алиасу. Тема без сущностей — сообщение «нет объектов,
...
сбрасывает ожидание и обрабатывается как обычный запрос.
