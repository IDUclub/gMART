...
def fetch_public(link: str, dest: Path) -> int:
    meta = requests.get(PUBLIC_META, params={"public_key": link, "limit": 10000},
                        timeout=60)
    meta.raise_for_status()
    m = meta.json()
    href = requests.get(PUBLIC_API, params={"public_key": link},
...
def fetch_private(link: str, dest: Path, token: str) -> int:
    ...
    m = requests.get(PRIVATE_META, params={"path": path}, headers=hdr,
                     timeout=60).json()
    href = requests.get(PRIVATE_API, params={"path": path}, headers=hdr,
                        timeout=60).json()["href"]
    return _save_resource(href, dest, m.get("type") == "file", m.get("name", ""))
...
def main():
    ...
    print(f"\nmanifest -> {man_path}")
    print(f"links: {ok} downloaded/cached, {need} need a Yandex token, "
          f"{len(status)-ok-need} errored")
    if need:
        print("  → provide YANDEX_OAUTH_TOKEN, or re-share those folders as "
